...
public class CreateUserAccountScreen extends Screen
{
    ...
    public void render()
    {
        ...
        Credential newCredential = new Credential(username,password,ssn);
        Customer newCustomer = new Customer(firstName, lastName, ssn, email, phone,newCredential);
        Address newAddress = new Address(unit,street,city,state,zip,newCustomer.getSsn());

        orm.saveNewData(newCustomer);
        orm.saveNewData(newCredential);
